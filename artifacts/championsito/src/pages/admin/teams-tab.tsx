...
export function TeamsTab() {
  ...
  const [settingChampionId, setSettingChampionId] = useState<number | null>(null);
  ...
  const handleToggleChampion = (team: Team) => {
    const next = !team.isChampion;
    setSettingChampionId(team.id);
    updateTeam.mutate(
      { id: team.id, data: { isChampion: next } },
      {
        onSuccess: () => {
          toast({
            title: next
              ? `${team.name} incoronata campione (+5 punti bonus)`
              : `Bonus campione rimosso da ${team.name}`,
          });
          queryClient.invalidateQueries({ queryKey: getListTeamsQueryKey() });
          setSettingChampionId(null);
        },
        onError: () => setSettingChampionId(null),
      },
    );
  };
  ...
  const currentChampion = teams?.find(t => t.isChampion);
          ...
          {potTeams.map(team => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className={`font-medium truncate ${team.isChampion ? 'text-amber-300' : ''}`}>
                  {team.name}
                </span>
                {team.isChampion && (
              ...
              <div className="flex gap-1 shrink-0">
                <Button
                  ...
                  title={team.isChampion ? 'Rimuovi bonus campione' : 'Segna come campione (+5 pt)'}
                  disabled={settingChampionId === team.id}
                  onClick={() => handleToggleChampion(team)}
                >
                  {settingChampionId === team.id
                ...
                <Button
                  ...
                  onClick={() => { setEditingTeam(team); setIsDeleteOpen(true); }}
                >
        ...
        <div>
          ...
          {currentChampion && (
            <p className="text-sm text-amber-300 mt-1 flex items-center gap-1">
              ...
              Campione: <span className="font-semibold">{currentChampion.name}</span>
              <span className="text-amber-400/70">(+5 pt bonus)</span>
            </p>
          )}
        </div>
